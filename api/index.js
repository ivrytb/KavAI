export default async function handler(req, res) {
    ...
    try {
        if (!voiceResult) {
            return res.status(200).send(`read=t-שלום,, אני המוח,, במה אוכל לעזור?=voice_result,,record,,,no,,,,20`);
        }
        ...
        console.log("--- שלב 1: הורדה ---");
        const audioResp = await fetch(downloadUrl);
        const buffer = Buffer.from(await audioResp.arrayBuffer());

        console.log("--- שלב 2+3: העלאה לגוגל ---");
        const startUpload = await fetch(`https://generativelanguage.googleapis.com/upload/v1/files?key=${API_KEY}`, {
            method: 'POST',
            headers: {
        ...
        const response = await fetch(geminiUrl, {
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [
                        { text: "אתה עוזר קולי בשם המוח. ענה בעברית ללא גרשיים. נקודות הופכות לפסיקים כפולים (,,). מקפים הופכים לרווח. ללא ירידות שורה." },
        ...
        console.log("תגובה מגוגל:", JSON.stringify(data));
        ...
        console.log("תשובה מעובדת:", aiText);
        ...
        return res.status(200).send(`read=t-חלה שגיאה,, ${error.message.split(' ').slice(0,5).join(' ')}=voice_result,,record,,,no,,,,20`);
    }
}
